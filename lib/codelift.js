...
var ConfirmRepoView = require('./confirm-repo-view.js')
...
module.exports = {
  activate: function(state) {
    ...
    confirmRepoView = new ConfirmRepoView(state.viewState);
    confirmRepoModal = atom.workspace.addModalPanel({
      item: confirmRepoView.getElement(),
      visible: false
    });
  },
...
getBranch = function () {
  ...
  fs.access(gitPath, function(){
    ...
    confirmRepoModal.show();
    confirmRepoView.setValues(provider, repository, branch)
    confirmRepoView.initialize(send)
  })
}
...
send = function (provider, repository, branch) {
  confirmRepoModal.hide();
  var form = {
    analysis: {
      provider: provider,
