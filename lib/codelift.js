...
module.exports = {
  activate: function(state) {
    ...
    signInView = SignInView(state.viewState)
    modal = atom.workspace.addModalPanel({
      item: this.signInView.getElement(),
      visible: false
    });
  },
...
signIn = function () {
  ...
  var email, password;
  // vscode.window.showInputBox({prompt: 'What is your Codelift e-mail?'})
  // .then(function(val){
    email = "colinwwiley@gmail.com"
    // vscode.window.showInputBox({prompt: 'What is your Codelift password?'})
    // .then(function(val){
      password = "password"
        var form = {
        session: {
